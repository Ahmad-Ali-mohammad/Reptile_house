      ...
      <nav className="bg-white/10 backdrop-blur-lg border-b border-white/20 shadow-lg">
        <div className="container mx-auto px-4 h-20 flex items-center justify-between">
            <a href="#" onClick={(e) => handleLinkClick(e, 'home')} className="flex items-center gap-3">
                <div className="w-10 h-10 bg-amber-500 rounded-xl flex items-center justify-center shadow-lg"><SnakeIcon className="w-6 h-6 text-gray-900" /></div>
                        ...
                        {isUserMenuOpen && (
                            <div className="absolute start-0 mt-2 w-52 bg-gray-900 border border-white/10 rounded-2xl shadow-2xl py-2 z-50 overflow-hidden">
                                ...
                                {(user.role === 'admin' || user.role === 'manager') && (
                                    <a href="#" onClick={(e) => { e.preventDefault(); setAppMode('dashboard'); setIsUserMenuOpen(false); }} className="flex items-center px-4 py-2 text-sm text-amber-400 font-black hover:bg-amber-500/10"><DashboardIcon className="w-4 h-4 me-3" /> لوحة الإدارة</a>
                                )}
