  ...
  if (variant === 'list') {
    return (
        <div 
            ...
            className="glass-medium rounded-3xl flex items-center p-6 transition-all duration-500 hover:shadow-[0_20px_40px_rgba(0,0,0,0.4)] hover:-translate-y-2 group border border-white/5 cursor-pointer relative overflow-hidden"
        >
            ...
            <div className="w-32 h-32 flex-shrink-0 overflow-hidden rounded-[2rem] border border-white/10 shadow-2xl relative z-10">
            ...
            <div className="flex-1 px-8 text-right relative z-10">
                <div className="flex items-center gap-3 mb-2">
                     <h3 className="text-2xl font-black tracking-tight group-hover:text-amber-400 transition-colors">{reptile.name}</h3>
                ...
                <div className="flex items-center gap-2 mt-3">
                    ...
                    <span className="font-poppins font-black text-sm">{reptile.rating}</span>
                </div>
            </div>
            <div className="flex flex-col items-end gap-3 relative z-10">
                <p className="text-3xl font-black text-amber-500 font-poppins">${reptile.price}</p>
                <button 
                    onClick={handleAddToCart}
                    disabled={reptile.status === 'غير متوفر'}
                    className="p-4 bg-amber-500 text-gray-900 rounded-2xl hover:bg-amber-400 transition-all shadow-xl active:scale-90 disabled:opacity-50 disabled:cursor-not-allowed"
                >
    ...
    >
      ...
      <div className="relative aspect-[1/1] overflow-hidden">
        ...
        <button
            ...
            className="absolute top-6 left-6 bg-black/40 backdrop-blur-xl p-3.5 rounded-2xl text-white hover:text-amber-400 hover:scale-110 transition-all duration-300 z-10 border border-white/10 shadow-xl"
        >
            {isWishlisted ? <HeartIconSolid className="w-5 h-5 text-amber-400"/> : <HeartIconOutline className="w-5 h-5"/>}
        </button>
        
        <div className={`absolute top-6 right-6 ${getStatusColor(reptile.status)} shadow-2xl text-[9px] font-black px-4 py-2 rounded-full uppercase tracking-widest z-10`}>
            {reptile.status}
        </div>

        <div className="absolute bottom-6 right-6 flex items-center gap-2 glass-dark px-4 py-2 rounded-full border border-white/10 z-10">
            <StarIcon className="h-3.5 w-3.5 text-amber-400" />
            <span className="text-white text-xs font-black font-poppins">{reptile.rating.toFixed(1)}</span>
        </div>
      </div>
      ...
      <div className="p-8 space-y-6 relative z-10">
        <div className="flex justify-between items-start">
            <div className="space-y-1">
                <h3 className="text-2xl font-black text-white tracking-tighter group-hover:text-amber-400 transition-colors leading-none">{reptile.name}</h3>
                <p className="text-gray-500 text-xs font-bold font-poppins uppercase tracking-widest">{reptile.species}</p>
            </div>
            <div className="text-left">
                <p className="text-3xl font-black text-amber-400 font-poppins tracking-tighter">${reptile.price}</p>
            </div>
        </div>
