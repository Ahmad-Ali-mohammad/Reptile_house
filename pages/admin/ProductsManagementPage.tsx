...
const ProductsManagementPage: React.FC = () => {
    ...
    const handleSave = (e: React.FormEvent) => {
        e.preventDefault();
        if (editingProduct) {
            const finalCategory = isNewCategory ? customCategory : (editingProduct.category || 'snake');
            const finalSpecies = isNewSpecies ? customSpecies : (editingProduct.species || existingSpecies[0]);

            if (!finalCategory || !finalSpecies || !editingProduct.imageUrl) {
            ...
            const productToSave: Reptile = {
                ...
                species: finalSpecies as string,
                    ...
                    >
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-10 text-right">
                            <div className="md:col-span-1 space-y-6">
                                <label className="block text-xs font-black text-amber-500 uppercase tracking-widest">الصورة التعريفية</label>
                                <div 
                            ...
                            <div className="md:col-span-2 grid grid-cols-1 sm:grid-cols-2 gap-6">
                                <div className="col-span-2">
                                    <label className="text-xs font-black text-amber-500 uppercase mb-2 block">الاسم</label>
                                    <input required className="w-full bg-[#1a1c23] border border-white/10 rounded-2xl py-4 px-6 text-white font-bold" value={editingProduct?.name || ''} onChange={e => setEditingProduct({...editingProduct, name: e.target.value})} />
                                </div>
                                <div>
                                    <label className="text-xs font-black text-amber-500 uppercase mb-2 block">الفصيلة</label>
                                    <select className="w-full bg-[#1a1c23] border border-white/10 rounded-2xl py-4 px-6 text-white" value={editingProduct?.species} onChange={e => setEditingProduct({...editingProduct, species: e.target.value})}>
                                        {existingSpecies.map(s => <option key={s} value={s}>{s}</option>)}
                                    </select>
                                </div>
                                <div>
                                    <label className="text-xs font-black text-amber-500 uppercase mb-2 block">السعر</label>
                                    <input type="number" className="w-full bg-[#1a1c23] border border-white/10 rounded-2xl py-4 px-6 text-white font-bold font-poppins" value={editingProduct?.price || 0} onChange={e => setEditingProduct({...editingProduct, price: Number(e.target.value)})} />
                                </div>
                                <div className="col-span-2">
                                    <label className="text-xs font-black text-amber-500 uppercase mb-2 block">الوصف</label>
                                    <textarea rows={4} className="w-full bg-[#1a1c23] border border-white/10 rounded-2xl py-4 px-6 text-white resize-none" value={editingProduct?.description || ''} onChange={e => setEditingProduct({...editingProduct, description: e.target.value})} />
                                </div>
                            </div>
                        </div>
