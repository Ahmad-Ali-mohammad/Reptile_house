        ...
        <div className="animate-fade-in space-y-10 text-right pb-20">
            <div className="flex flex-col md:flex-row justify-between items-center gap-6">
                <div>
                    ...
                    <p className="text-gray-500 font-bold">تحكم في الصور والعناوين التي تظهر في الواجهة الرئيسية.</p>
                </div>
                <button 
                        ...
                        <div className="p-10 flex-1 space-y-6">
                            ...
                            <div className="flex items-center justify-between pt-6 border-t border-white/5">
                                ...
                                <div className="flex gap-3">
                                    <button onClick={() => handleOpenModal(slide)} className="p-3 bg-white/5 text-amber-500 rounded-xl hover:bg-white/10 transition-all"><EditIcon className="w-5 h-5" /></button>
                                    <button onClick={() => deleteHeroSlide(slide.id)} className="p-3 bg-red-500/10 text-red-400 rounded-xl hover:bg-red-500 hover:text-white transition-all"><TrashIcon className="w-5 h-5" /></button>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <form onSubmit={handleSave} className="relative w-full max-w-4xl glass-dark border border-white/10 rounded-[3.5rem] p-12 space-y-8 animate-scale-in max-h-[90vh] overflow-y-auto custom-scrollbar">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                            <div className="space-y-6">
                                <div>
                                    <label className="text-[10px] font-black text-amber-500 uppercase mb-2 block">عنوان الشريحة</label>
                                    <input required className="w-full bg-[#1a1c23] border border-white/10 rounded-2xl p-4 text-white font-bold" value={editingSlide?.title || ''} onChange={e => setEditingSlide({...editingSlide, title: e.target.value})} />
                                </div>
                                <div>
                                    <label className="text-[10px] font-black text-amber-500 uppercase mb-2 block">النص الوصفي</label>
                                    <textarea rows={3} required className="w-full bg-[#1a1c23] border border-white/10 rounded-2xl p-4 text-white font-bold resize-none" value={editingSlide?.subtitle || ''} onChange={e => setEditingSlide({...editingSlide, subtitle: e.target.value})} />
                                </div>
                                <div>
                                    <label className="text-[10px] font-black text-amber-500 uppercase mb-2 block">رابط الصورة</label>
                                    <input required className="w-full bg-[#1a1c23] border border-white/10 rounded-2xl p-4 text-white font-bold" value={editingSlide?.image || ''} onChange={e => setEditingSlide({...editingSlide, image: e.target.value})} />
                                </div>
                            </div>

                            <div className="space-y-6">
                                <div>
                                    <label className="text-[10px] font-black text-amber-500 uppercase mb-2 block">نص الزر</label>
                                    <input required className="w-full bg-[#1a1c23] border border-white/10 rounded-2xl p-4 text-white font-bold" value={editingSlide?.buttonText || ''} onChange={e => setEditingSlide({...editingSlide, buttonText: e.target.value})} />
                                </div>
                                <div>
                                    <label className="text-[10px] font-black text-amber-500 uppercase mb-2 block">الرابط</label>
                                    <select className="w-full bg-[#1a1c23] border border-white/10 rounded-2xl p-4 text-white font-bold" value={editingSlide?.link || 'showcase'} onChange={e => setEditingSlide({...editingSlide, link: e.target.value})}>
