...
import { EditIcon, TrashIcon, PlusIcon, SearchIcon, ImageIcon } from '../../components/icons';
        ...
        <div className="animate-fade-in space-y-8 text-right">
            <div className="flex flex-col md:flex-row justify-between items-center gap-6">
                <div>
                    ...
                    <p className="text-gray-500 font-bold">تحكم في المقالات التعليمية والأخبار التي تظهر للعملاء.</p>
                </div>
                            ...
                            <tr key={article.id} className="hover:bg-white/5 group transition-colors">
                                <td className="p-6">
                                    <div className="flex items-center gap-4">
                                        <img src={article.image} className="w-14 h-14 rounded-2xl object-cover shadow-lg border border-white/10" />
                                        <div>
                    ...
                    <form onSubmit={handleSave} className="relative w-full max-w-4xl glass-dark border border-white/10 rounded-[3rem] p-12 space-y-8 animate-scale-in max-h-[90vh] overflow-y-auto custom-scrollbar">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                            <div className="space-y-6">
                                <div>
                                    <label className="text-[10px] font-black text-amber-500 uppercase tracking-widest mb-2 block">عنوان المقال</label>
                                    <input required placeholder="اكتب عنواناً جذاباً..." className="w-full bg-[#1a1c23] border border-white/10 rounded-2xl p-4 text-white font-bold outline-none focus:ring-2 focus:ring-amber-500" value={editingArticle?.title || ''} onChange={e => setEditingArticle({...editingArticle, title: e.target.value})} />
                                </div>
                                <div>
                                    <label className="text-[10px] font-black text-amber-500 uppercase tracking-widest mb-2 block">فئة المقال</label>
                                    <select className="w-full bg-[#1a1c23] border border-white/10 rounded-2xl p-4 text-white outline-none focus:ring-2 focus:ring-amber-500" value={editingArticle?.category} onChange={e => setEditingArticle({...editingArticle, category: e.target.value as any})}>
                                ...
                                <div>
                                    <label className="text-[10px] font-black text-amber-500 uppercase tracking-widest mb-2 block">رابط الصورة (من مكتبة الوسائط)</label>
                                    <input required placeholder="الصق رابط الصورة هنا..." className="w-full bg-[#1a1c23] border border-white/10 rounded-2xl p-4 text-white outline-none focus:ring-2 focus:ring-amber-500" value={editingArticle?.image || ''} onChange={e => setEditingArticle({...editingArticle, image: e.target.value})} />
                                </div>
                            </div>

                            <div className="space-y-6">
                                <div>
                                    <label className="text-[10px] font-black text-amber-500 uppercase tracking-widest mb-2 block">ملخص قصير (Excerpt)</label>
                                    <textarea required rows={4} placeholder="يظهر هذا النص في المعرض الرئيسي للمدونة..." className="w-full bg-[#1a1c23] border border-white/10 rounded-2xl p-4 text-white outline-none focus:ring-2 focus:ring-amber-500 resize-none" value={editingArticle?.excerpt || ''} onChange={e => setEditingArticle({...editingArticle, excerpt: e.target.value})} />
                                </div>
                            </div>

                            <div className="md:col-span-2">
                                <label className="text-[10px] font-black text-amber-500 uppercase tracking-widest mb-2 block">المحتوى الكامل للمقال</label>
                                <textarea required rows={10} placeholder="اكتب المقال كاملاً هنا... يمكنك شرح كافة التفاصيل التعليمية." className="w-full bg-[#1a1c23] border border-white/10 rounded-3xl p-6 text-white outline-none focus:ring-2 focus:ring-amber-500 resize-none leading-loose" value={editingArticle?.content || ''} onChange={e => setEditingArticle({...editingArticle, content: e.target.value})} />
                            </div>
                        </div>
