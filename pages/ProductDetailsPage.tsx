...
import { StarIcon, HeartIconOutline, HeartIconSolid, ChevronRightIcon, ShoppingCartIcon } from '../components/icons';
            ...
            <div className="glass-medium p-10 md:p-14 rounded-[3rem] border border-white/10 animate-fade-in shadow-2xl text-right relative overflow-hidden">
               ...
               <div className="text-gray-200 text-xl leading-loose space-y-6">
                {reptile.description ? (
                    ...
                    <p>هذا المخلوق الرائع تم اختياره بعناية فائقة من قبل خبيرنا سيمون شلش. يتميز بصحة ممتازة، ونمط لوني فريد يجعله قطعة فنية حية في أي مكان يوضع فيه. لقد تم تعويده على التغذية المنتظمة وفحصه طبياً لضمان خلوه من الطفيليات والمشاكل الصحية الشائعة.</p>
                )}
            ...
            {otherRecent.length > 0 && (
                <section className="mt-40 border-t border-white/5 pt-20">
                    <div className="flex justify-between items-end mb-16 text-right flex-row-reverse">
                        <h2 className="text-4xl font-black tracking-tighter text-white">زواحف شاهدتها مؤخراً</h2>
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-8">
                        {otherRecent.map((recent, idx) => (
                            <div key={recent.id} className="scale-95 opacity-80 hover:scale-100 hover:opacity-100 transition-all duration-500">
                                <ReptileCard reptile={recent} setPage={setPage} index={idx} />
                            </div>
                        ))}
                    </div>
                </section>
